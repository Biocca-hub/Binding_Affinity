...
def main() -> int:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=args.num_workers) as executor:
            future_to_pair = {
                executor.submit(run_foldseek, file1, file2, out_file, tmp_dir): (file1, file2)
                for file1, file2, out_file, tmp_dir in jobs
            }
            for future in tqdm(as_completed(future_to_pair), total=len(future_to_pair), desc="foldseek jobs"):
